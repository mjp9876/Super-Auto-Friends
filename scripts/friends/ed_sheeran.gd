		...
		while not done:
			...
			while not shopScene.friend_shop_slots[shop_slots[0]].active:
				...
				if shop_slots.size() == 0:
					...
					break
			if not done:
				shopScene.friend_shop_slots[shop_slots[0]].card.colour = colours.BLUE
				shopScene.friend_shop_slots[shop_slots[0]].card.setColour()
				Manager.move_symbol(global_position, shopScene.friend_shop_slots[shop_slots[0]].global_position, blue_icon, "")
				done_changes += 1
				shop_slots.pop_front()
