...
func start_turn():
	if not blocked_ability:
		...
		if target_friend1 != null:
			proc()
			target_friend1.hp += 3
			Manager.move_symbol(global_position, target_friend1.global_position, most_hp_target_texture, "3")
			target_friend1.setStatText()
			if target_friend2 != null:
				await get_tree().create_timer(0.2).timeout
				target_friend2.hp += 3
				Manager.move_symbol(global_position, target_friend2.global_position, most_hp_target_texture, "3")
